...
function HotelCard({room,index}) {
  return (
    // Here we will mount each hotel card with the room data and scroll to the top when clicked.
    <Link to={'/rooms/'+room._id} 
      onClick={() => window.scrollTo(0, 0)}
      key={room._id} 
      className='relative max-w-70 w-full rounded-xl overflow-hidden
         bg-white text-gray-500/90 shadow-[0px_4px_4px_rgba(0,0,0,0.05)]'
      >
        
        <img src={room.images[0][0]} alt='room_image' />
        ...
        <div className='p-4 pt-5'>
            <div className='flex items-center justify-between'>
                ...
                <div className='flex items-center gap-1'>
                    <img src={assets.starIconFilled} alt='star_icon' /> 4.5
                </div>
            </div>
