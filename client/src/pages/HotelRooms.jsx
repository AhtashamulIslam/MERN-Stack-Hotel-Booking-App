...
//We describe the function for the filters of hotel rooms. 

// We create a checkbox component to select the filters.
...
// We create Radio button component to select the sort options.
...
function HotelRooms() {
    const { destination } = useParams(); // Assuming you're using react-router-dom to get the city from the URL.
    const navigate = useNavigate();
    const [openFilters, setOpenFilters] = useState(false);

    const [rooms, setRooms] = useState([]); // Using dummy data for initial state.
...
// Function to handle filter and sorting changes.
    ...
    const filterDestination = (room)=>{
        ...
        return room.cityName.toLowerCase().includes(destination.toLowerCase());
    }
    ...
    // Clear all filters and sort options.
    const clearFilters = () => {
        setSelectedFilters({
            roomTypes: [],
            priceRanges: []
     ...
     const fetchRooms = async () => {
            try {
                const res = await fetch(`/api/room/hotel-rooms/${destination}`);
  ...
  return (
    <div className='flex flex-col-reverse lg:flex-row items-start 
                    justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32 bg-white'>
        <div>
            <div className='flex flex-col items-start text-left'>
                <h1 className='font-playfair text-black text-4xl md:text-[40px]'>Hotel Rooms</h1>
                <p className='text-sm md:text-base text-gray-500/90 mt-2 max-w-174'>Take advantage of our limited-time offers and special packages to 
                    enhance your stay and create unforgettable memories. </p>
            </div>
            {filteredRooms.map((room) => (
                ...
                key={room._id}>
                    ...
                    src={room.images[0][0]} alt='room_image' title='View Room Details' 
                    className='md:w-1/2 max-h-65 object-cover rounded-xl shadow-lg cursor-pointer' />
                    <div className='md:w-1/2 flex flex-col gap-2'>
                        <p className='text-gray-500'>{ room.hotelAddress.split(',')[1]}</p>
                        <p onClick={()=> {
                        navigate(`/rooms/${room._id}`)
                        scrollTo(0, 0);
                         }}
        ...
        {/*Filters*/}
        <div className='bg-white border w-80 border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16'>
